    ...
    <div className={`fixed bg   bg-white border-l-2 overflow-y-scroll right-0 top-[67px] pt-3 hide-scrollbar bottom-0 md:w-1/3 b-white l text-gray-900  w-full ${isOpen ? 'translate-x-0 transition-all duration-200' : 'transition-all duration-200 translate-x-[100%]'}`}>
         ...
         <div className="over ">
            {/* <div className="flex  items-center  justify-between">
             <button onClick={onClose} className="text underline block text-gray-700"><X className="clas size-12"/></button>
        <div className="block">

        <button onClick={clearCart} className="mt-4 bg-red-50red-500 bg-opacity-15  flex i items-cente px-4 py-2 rounded"><TrashIcon className="size-5 line inline"/>Vider le panier</button>
        </div>
        </div> */}
          ...
          <div className="pb  px-2 ">
            <div className="px-3">
              ...
              <p className="flex gap-1 items-center my-4 text-gray-500">
                ...
                Il est préfèrable de prendre votre temps pour 
                vérifier correctement les articles ajoutés dans le
                panier avant de procéder à la commande
              </p>
              </div>
            {cart.map(item => (
              <>
...
<p className="text-lg">${item.price}</p>
    ...
    <div className="flex justify-between  items-center my- ">
        ...
        <Link href="/products">
              <button  className="mx-auto w-max block  underline text-gray-700">
                    OU CONTINUER LE SHOPING 
              </button>
        </Link>
